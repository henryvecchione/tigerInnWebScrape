...
def main():
  ...
  while True:
    try:
      ...
      print('Not logged in', end='\r')
      continue
  
  
  # once logged in, begin scraping loop
  years = range(2021, 2022) # grad years
  for year in years:
    print(f'Year: {year}')
    ...
    while True:
      try:
        numResults = web.find_element(By.XPATH, '/html/body/form/div[3]/main/section/div/div/section/div[3]/div[1]/div/div[2]/div[2]/p/strong').text
    ...
    while True:
      ...
      # (6) Iter through links for that page
      for link in links:
        # (6.1) visit site
        web.get(link)
        ...
        print('*--------------------*')
      if pageNo != numPages:
        newUrl = '/'.join(url.split('/')[:-1]) + '/' + str(pageNo + 1) + '#gridStart'
        web.get(newUrl)
